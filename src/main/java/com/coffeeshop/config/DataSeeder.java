...
public class DataSeeder implements CommandLineRunner {
        ...
        private void seedUsers() {
                ...
                createStaff("manager", "Nguyen Van An", "manager@coffee.com", "0901234567", Role.ADMIN, 50000.0, "S001");
                createStaff("barista1", "Tran Thi Binh", "barista1@coffee.com", "0901112222", Role.STAFF, 35000.0, "S002");
                createStaff("barista2", "Le Van Cuong", "barista2@coffee.com", "0903334444", Role.STAFF, 35000.0, "S003");
                createStaff("server1", "Pham Thi Dung", "server1@coffee.com", "0905556666", Role.STAFF, 25000.0, "S004");
        ...
        private void seedHistory() {
                ...
                for (int i = 9; i >= 0; i--) {
                        ...
                        if (currentMonth.getMonthValue() == now.getMonthValue() && currentMonth.getYear() == now.getYear()) {
                                maxDay = now.getDayOfMonth();
                        }

                        for (int j = 0; j < ordersCount; j++) {
                                com.coffeeshop.entity.Order order = new com.coffeeshop.entity.Order();
                                order.setUser(!allUsers.isEmpty() ? allUsers.get(rand.nextInt(allUsers.size())) : admin);
                                ...
                                order.setCreatedAt(currentMonth.withDayOfMonth(day).withHour(8 + rand.nextInt(12)).withMinute(rand.nextInt(60)));
                                ...
                                for (int k = 0; k < itemsCount; k++) {
                                        ...
                                        com.coffeeshop.entity.OrderItem d = new com.coffeeshop.entity.OrderItem();
                                        d.setOrder(order);
                                        d.setProduct(p);
                                        d.setSnapshotProductName(p.getName());
                                        d.setQuantity(1 + rand.nextInt(2));
                                        double price = 40000.0 + rand.nextInt(30) * 1000;
                                        d.setSnapshotUnitPrice(BigDecimal.valueOf(price));
                                        d.setSubTotal(BigDecimal.valueOf(price * d.getQuantity()));
                                ...
                                for (com.coffeeshop.entity.OrderItem d : details) {
                                        d.setOrder(saved);
                                        orderItemRepository.save(d);
                                }
                        }
        ...
        private void seedShifts() {
                ...
                for (int i = 1; i < 14; i++) {
                        ...
                        for (com.coffeeshop.entity.User u : staff) {
                                if ("admin".equals(u.getUsername())) continue;
        ...
        private void createShift(com.coffeeshop.entity.User user, java.time.LocalDateTime start,
                        java.time.LocalDateTime end, Double revenue) {
                ...
                s.setStatus(end == null ? com.coffeeshop.entity.ShiftStatus.OPEN : com.coffeeshop.entity.ShiftStatus.CLOSED);
        ...
        private void seedActiveData() {
                ...
                if (manager != null) createShift(manager, now.minusHours(4), null, null);
                if (barista1 != null) createShift(barista1, now.minusHours(2), null, null);

                for (int i = 0; i < 5; i++) {
                        com.coffeeshop.entity.Order order = new com.coffeeshop.entity.Order();
                        order.setUser(barista1 != null ? barista1 : userRepository.findByUsername("admin").orElse(null));
                        order.setCustomerName("Active Customer " + (i + 1));
                        order.setOrderType("POS Order");
                        com.coffeeshop.entity.OrderStatus st = (i % 2 == 0)
                                ? com.coffeeshop.entity.OrderStatus.PENDING
                                : com.coffeeshop.entity.OrderStatus.CONFIRMED;
                        order.setStatus(st);
                        order.setOrderStatus(st.name());
                        ...
                        for (int k = 0; k < itemsCount; k++) {
                                ...
                                d.setSnapshotUnitPrice(BigDecimal.valueOf(price));
                                d.setSubTotal(BigDecimal.valueOf(price));
                                total += price;
                                details.add(d);
                        }
                        order.setTotalAmount(total);
        ...
        private void seedJobs() {
                if (jobPostingRepository.count() == 0) {
                        ...
                        job1.setDescription("We are looking for an experienced Barista to join our team.");
                        job1.setRequirements("2+ years experience, Latte Art skills.");
                        job1.setActive(true);
                        ...
                        job3.setDescription("Join our vibrant team as service staff.");
        ...
        private void seedProducts() {
                ...
                createProduct("Cafe Latte", "Cà phê Latte", "Creamy espresso with steamed milk.", "Espresso kem mịn với sữa nóng.",
                        "milk,sữa,cream,kem,espresso,coffee,cà phê,latte,sweet,ngọt,hot,nóng", "/images/products/CaffeLatte.png", coffee, 55000.0);
                createProduct("Espresso", "Espresso", "Intense double-shot espresso.", "Espresso đậm đà hai shot.",
                        "espresso,coffee,cà phê,strong,mạnh,bitter,đắng,black,đen,bold,shot,hot,nóng", "/images/products/Espresso.png", coffee, 45000.0);
                createProduct("Peach Tea", "Trà Đào", "Refreshing peach tea with fresh peach slices.", "Trà đào thanh mát.",
                        "tea,trà,peach,đào,fruit,trái cây,refreshing,mát,iced,đá,cold,sweet,ngọt", "/images/products/PeachTea.png", tea, 55000.0);
                createProduct("Sakura Blossom Tea", "Trà Hoa Anh Đào", "Delicate cherry blossom infused tea.", "Trà hoa anh đào.",
                        "tea,trà,sakura,cherry blossom,hoa anh đào,floral,hoa,light,nhẹ,elegant,hot,nóng", "/images/products/SakuraBlossomTea.png", tea, 58000.0);
                createProduct("Strawberry Smoothie", "Sinh tố Dâu", "Thick strawberry smoothie blended with fresh milk.", "Sinh tố dâu.",
                        "smoothie,sinh tố,strawberry,dâu,milk,sữa,cream,kem,sweet,ngọt,cold,lạnh,ice,fruit", "/images/products/StrawberrySmoothie.png", smoothie, 60000.0);
                createProduct("Coconut Juice", "Nước Dừa", "Fresh young coconut water with coconut jelly.", "Nước dừa tươi non.",
                        "coconut,dừa,juice,nước,fresh,tươi,natural,tự nhiên,cold,lạnh,ice,healthy", "/images/products/CoconutJuice.png", juice, 50000.0);
        ...
        private void createProduct(String name, String nameVi, String desc, String descVi,
                ...
                p.setCategory(cat);
                p.setAvailable(true);
                p.setBasePrice(BigDecimal.valueOf(basePrice));
                com.coffeeshop.entity.Product saved = productRepository.save(p);
                if (productSizeRepository.findByProductId(saved.getId()).isEmpty()) {
                        com.coffeeshop.entity.ProductSize s = new com.coffeeshop.entity.ProductSize("Standard", basePrice, saved);
        ...
        private void seedIngredients() {
                ...
                System.out.println("✓ Ingredients seeded successfully.");
        }
        ...
        private void seedRecipes() {
                if (productRecipeRepository.count() > 0) return;
                createRecipe("Cafe Latte", "Espresso Shot", 1.0);
                createRecipe("Cafe Latte", "Whole Milk", 200.0);
                createRecipe("Cafe Latte", "Sugar Syrup", 20.0);
                createRecipe("Espresso", "Espresso Shot", 2.0);
                createRecipe("Peach Tea", "Black Tea Leaves", 5.0);
                createRecipe("Peach Tea", "Peach Puree", 40.0);
                createRecipe("Peach Tea", "Sugar Syrup", 30.0);
                createRecipe("Peach Tea", "Ice", 150.0);
                ...
                createRecipe("Coconut Juice", "Water", 200.0);
        ...
        private void createRecipe(String productName, String ingredientName, Double quantity) {
                productRepository.findAll().stream()
                        ...
                        .ifPresent(product -> ingredientRepository.findByName(ingredientName).ifPresent(ingredient -> {
